'use client';
// app/(frontend)/FrontendShell.tsx
// Client component — header, footer, whatsapp button
  ...
  return (
    <header
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
        scrolled ? 'backdrop-blur-md shadow-md' : 'backdrop-blur-sm'
      }`}
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-12 md:py-16">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-10">
          <div>
            <div className="flex flex-col items-start leading-tight mb-3">
              <div className="px-3 py-2 rounded-lg" style={{ backgroundColor: '#2d4a3e' }}>
                <span className="font-display text-xl font-bold tracking-wider" style={{ color: '#e4caa9' }}>
                  Subhiksham
                </span>
              </div>
            ...
            <p className="mt-2 text-sm text-brand-cream/80 leading-relaxed">
              Pure vegetarian South Indian cuisine, crafted with love and tradition in the heart of Palakkad.
            </p>
            <div className="flex items-center gap-2 mt-4">
              <span className="veg-dot" />
              <span className="text-xs text-brand-cream/70 tracking-wider">100% PURE VEGETARIAN</span>
            </div>
          </div>

          <div>
            <h4 className="font-display text-lg font-semibold tracking-wide mb-4">Quick Links</h4>
            <nav className="flex flex-col gap-2">
              {navLinks.map((link) => (
                <Link key={link.href} href={link.href} className="text-sm text-brand-cream/80 hover:text-brand-ochre transition-colors">
                  {link.label}
                </Link>
              ))}
            </nav>
          ...
          <div>
            <h4 className="font-display text-lg font-semibold tracking-wide mb-4">Visit Us</h4>
            <div className="space-y-3 text-sm text-brand-cream/80">
              <p>100 Ft. Road, Palakkad</p>
              <p>Kerala, India - 678001</p>
              <a href="tel:+919876543210" className="block hover:text-brand-ochre transition-colors">
                +91 98765 43210
              </a>
              <div className="pt-2">
                <p className="font-display text-sm font-semibold text-brand-cream mb-2">Opening Hours</p>
        ...
        <div className="mt-10 pt-6 border-t border-brand-cream/20 flex flex-col sm:flex-row items-center justify-between gap-4">
          <p className="text-xs text-brand-cream/60">
            &copy; {new Date().getFullYear()} Subhiksham - The Tiffin Club. All rights reserved.
          </p>
